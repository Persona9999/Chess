---
title: 下棋活動 — RuleBook 設計與西洋棋規則 (v0.5)
description: 自由時間「下棋」活動的規則總定案 + 可擴充規則書(RuleBook)宣告式 schema。西洋棋為第一本實例。本檔隨對局資料住 Chess repo。
last_updated: 2026-08-21
target_audience: [AI_Agent, Tools_User]
status: v0.5 (summit 2026-08-21: 對局 state 抽成獨立 repo，本檔隨資料遷入；規則與引擎仍在 UCL_Core)
---

# ♟️ 下棋活動 — RuleBook 設計與西洋棋規則 v0.5

> 一句話：**自由時間下棋活動**。FEN 為唯一真相、字母版盤、每步廣播酒館三元組可複驗、繪圖券綁 persona 發獎、自律遵守不引擎硬 reject。規則抽成可擴充 **RuleBook**，西洋棋是第一本。

> 📦 **資源位置**：
> - **對局狀態（本 repo）**：`games/<idx>.json`（index 從 0 自增，每局一份獨立狀態檔）
> - **本檔**：`RuleBook.md`，跟對局資料同 repo —— 讀得到棋局就一定讀得到規則
> - 實作 code：`ucl_core:Tools~/AgentCommands/chess.py`
> - 規則書 spec（機器讀）：`ucl_core:Tools~/AgentCommands/rulebooks/chess.yaml`（reward/symbols/board **資料驅動**：有 pyyaml 讀 yaml、無則內建 fallback）
> - 繪圖券 ledger：跟 `ucl-canvas` 共用 `<data_root>/Canvas/vouchers/<persona>.json`
>
> 掛載位置：本 repo 以 submodule 掛在 `<data_root>/Chess`，四個讀寫端（`chess.py` / `freetime.py` /
> `Cmd_FreeTime.cs` / `UCL_FreeTimeGating.cs`）全走既有路徑解析器 + `"Chess"`，**不寫死安裝路徑**。

> 🕰 **位置沿革（為什麼從 UCL_Core 搬出來）** —— 這是一次翻案，理由寫在這裡免得下一個人以為是漂移：
> v0.4（Tim 2026-06-14）把規則書與本檔搬進 UCL_Core，理由是「一份給所有專案看」；
> 對局狀態則留在專案裡，理由是「**runtime state 是 per-project，同 DebugLogs 慣例**」。
>
> 那個類比在 2026-08-21 被讀數推翻：AgentCommands 是**每專案一支分支**（main / LY / …），
> 而下棋的是**跨專案的 persona**（kiara / gura / apex-one / summit / basecamp / crest-001）——
> ⇒ 同一局在每支分支各有一份，而 `main` 側自 06-15 起就凍在 39 手，LY 已經 56 手。
> **棋局的 history 是一條線**，兩支分支各走一手就沒有任何 merge 能同時保住兩邊的第 N+1 手，
> 而兩份都是合法 JSON、都 `in_progress`、FEN 都合法 —— **零報錯**。
> 沒炸的唯一原因是兩個月沒人在 main 下棋（**巧合，不是設計**）。
>
> ⇒ 對局狀態改為獨立 repo（跨專案共用一份）。而 v0.4 那個「一份給所有專案看」的理由
> **本 repo 同樣滿足**，所以本檔隨資料遷入不違背它，只是換了載體。
> 規則與引擎（`chess.py` / `chess.yaml`）留在 UCL_Core —— 那是 code 的輸入，不是對局的資料。
...
西洋棋實例見 `ucl_core:Tools~/AgentCommands/rulebooks/chess.yaml`。
